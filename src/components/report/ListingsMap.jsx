            ...
            <div className="transition-transform duration-200 hover:scale-105">
              <div className="px-3 py-1 rounded-full bg-white shadow-lg border border-gray-100">
                <span className="text-sm font-semibold">
                  {`$${shortenNumber(property.listPrice)}`}
                </span>
              </div>
            </div>
          </Marker>
            ...
            <div className="px-3 py-1 rounded-full bg-white shadow-lg border border-gray-100">
              ...
              >
                {group.properties.length > 1
                  ...
                  : group.properties[0].uiStatus === 'Sold'
                  ? `$${shortenNumber(group.properties[0].soldPrice)}`
                  : `$${shortenNumber(group.properties[0].listPrice)}`}
              </span>
            </div>
          </div>
        </Marker>
    ...
    <div className="w-full h-[550px]">
      ...
      <div className="absolute top-1 left-1 transform z-20">
        <div className="flex flex-row bg-[grey] text-white rounded-lg md:space-y-0 md:space-x-1">
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all duration-200 ${
              activeView === 'list' ? 'bg-blue-500 text-white' : 'text-white-600'
            }`}
          >
            List
          </button>
      ...
      <div
        className={`absolute inset-0 transition-opacity duration-300 ${
          getVisibility('map') ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}
      >
        <Map
          {...viewState}
